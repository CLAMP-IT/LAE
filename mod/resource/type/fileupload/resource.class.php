<?php // $Id: resource.class.php,v 1.53.2.12 2007/05/16 21:26:16 skodak Exp $
...
class resource_fileupload extends resource_base {
    function resource_fileupload($cmid=0) {
    ...
    function add_instance($resource) {
        ...
        $course = $this->course;
        $resource->reference = clean_filename($_FILES['newreference']['name']);
        require($CFG->libdir.'/filelib.php');
        require($CFG->libdir.'/adminlib.php');
        require_once($CFG->dirroot.'/lib/uploadlib.php');
  
        $course->maxbytes = 0;  // We are ignoring course limits
        $um = new upload_manager('newreference',false,true,$course,false,0);

        // Workaround to ascertain what the actual filename will end up being
        $file = $_FILES['newreference'];
        $file['name'] = $resource->reference;
        $um->handle_filename_collision($CFG->dataroot . '/' . $course->id, $file, true);
        $resource->reference = $file['name'];

        // Store this as type = file so it gets treated as such from now on
        $resource->type = 'fileupload';  // changed to fileupload for interface consistency
  
        if ($um->process_file_uploads(make_upload_directory("$course->id"))) {
            // success
        } else {
	...
	// 2010-01-05 - dwheeler - clamp - fix for update issues - minor issue remains - we overwrite files with the same name
    function update_instance($resource) {
        global $CFG;
        $course = $this->course;
		$resource->reference = clean_filename($_FILES['reference']['name']);
        require($CFG->libdir.'/filelib.php');
        require($CFG->libdir.'/adminlib.php');
        require_once($CFG->dirroot.'/lib/uploadlib.php');

        if ($_FILES['newreference']['name'] != '') { // If a file was uploaded, get its info.
          $course->maxbytes = 0;  // We are ignoring course limits
	  // 2009-07-30 - dwheeler - clamp - add option to permit an update without a new file upload (added true,true to parameter list)
	  // upload_manager  ([$inputname = ''], [$deleteothers = false], [$handlecollisions = false], [$course = null], [$recoverifmultiple = false], [$modbytes = 0], [$silent = false], [$allownull = false], [$allownullmultiple = true]) 
          $um = new upload_manager('newreference',false,false,$course,false,0,true,true);

          // Workaround to ascertain what the actual filename will end up being
		  // 2009-12-04 - dwheeler - clamp - 
          $resource->reference = clean_filename($_FILES['newreference']['name']);
          $file = $_FILES['newreference'];
          $file['name'] = $resource->reference;
		// 2009-06-08 - dwheeler - clamp - we currently overwrite any existing file without warning
		//        $um->handle_filename_collision($CFG->dataroot . '/' . $course->id, $file, true);
		  // 2009-12-04 - dwheeler was newreference
          $resource->reference = $file['name'];
          $resource->type = 'fileupload';
	...
	else { // if no file was uploaded, use the previously-uploaded file
	  $resource->reference = $_POST['reference']; 
	} 
 ...
 /**
    ...
    */
        ...
        /// display the resource into a object tag
            ...
            /// display the resource into a frame tag
              ...
              if (!empty($localpath)) {  // Show it like this so we interpose some HTML
                  echo "<frame src=\"view.php?id={$cm->id}&amp;type={$resource->type}&amp;inpopup=true\" title=\"".
                        get_string('modulename','resource')."\"/>";
              } else {
	...
	// set up form element to hold the current filename  - CURRENTLY COMPLETELY BROKEN!! CKM 2009-10-01!
	$mform->addElement('hidden', 'reference'); 


	// form element for uploading new file
        $mform->addElement('file', 'newreference', get_string('attachment', 'resource'));
        $mform->setDefault('newreference', $CFG->resource_defaulturl);
        ...
        $mform->setHelpButton('forcedownload', array('forcedownload', get_string('forcedownload', 'resource'), 'resour
ce'));
	$mform->setDefault('forcedownload', 0); 
        $mform->setAdvanced('forcedownload');
